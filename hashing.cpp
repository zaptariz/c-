...
int main(int argc, char* argv[])
{
	...
	block = convert_to_binary(message); //message=baggi
...
vector<unsigned long> resize_block(vector<unsigned long> input)
{
	...
	for(int i = 0; i < 64; i = i + 4)
	{
		...
		cout<<((unsigned long) input[i] << 24)<<" checking"<<endl;
	
		cout<<temp<<" temp"<<endl;
		temp |= (unsigned long) input[i + 1] << 16;// temp=temp|input[i+1]<<16
		...
		cout<<temp<<endl;
		cout<<temp.to_ulong()<<" value after the bitwise process"<<endl<<endl;
	}
	cout<<output.size()<<endl;
	cout<<sizeof(output)<<" output.sizeof()"<<endl;
	return output;
}
...
vector<unsigned long> convert_to_binary(const string input)
{
	...
	vector<unsigned long> block;
    cout<<input.size()<<" length of message=baggi"<<endl;
	// For each character, convert the ASCII chararcter to its binary
	// representation.
	for (int i = 0; i < input.size(); ++i)
	{
		// Make a temporary variable called B sto store the 8 bit pattern
		// for the ASCII value.
		bitset<8> b (input[i]);
		cout<<input[i]<<" value of c_str()[i]"<<endl;
		cout<<b<<" bitset of b"<<endl;
		// Add that 8 bit pattern into the block.
		block.push_back(b.to_ulong());
		cout<<b.to_ulong()<<" value of b.to_ulong()"<<endl;
		cout<<block.size()<<" block size"<<endl<<endl;
	}
...
vector<unsigned long> pad_to_512bits(vector<unsigned long> block)
{
	...
	cout<<sizeof(block.size())<<" block.size() in byte"<<endl;
	...
	cout<<"("<< big_64bit_blob <<")"<<endl<<" bits of big_64bit_blob" <<endl;
	...
	block.push_back(temp_string_holder1.to_ulong()); //to_ulong means give number value eg A=65
	
	cout<<temp_string_holder1.to_ulong()<<" value of temp_string_holder1.to_ulong()"<<endl;
	int e=0;
...
string compute_hash(const vector<unsigned long> block)
{
	...
	unsigned long W[64];
	cout<<block.size()<<" block size in the compute hash"<<endl;
	for(int t = 0; t < 16; t++)
	{
